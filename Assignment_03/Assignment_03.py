...
class Customer:
    ...
    def membership_discount(self, subtotal):
        rate = self.high_discount_rate if subtotal > DISCOUNT_THRESHOLD else self.low_discount_rate
...
class Order:
    ...
    def receipt(self):
        ...
        lines.extend(str(item) for item in self.items)
        lines.extend([
            "-" * RECEIPT_WIDTH,
...
PRODUCT_OBJECTS = [Product(name, price, category) for name, price, category in PRODUCTS]
...
def build_orders():
    ...
    for name, tier, raw_items in ORDERS:
        ...
        items = [OrderItem(PRODUCT_OBJECTS[index], quantity) for index, quantity in raw_items]
